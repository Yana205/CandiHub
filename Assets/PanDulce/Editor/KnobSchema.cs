    ...
    public static class KnobSchema
    {
        ...
        static Section[] BuildSections() => new[]
        {
            ...
            new Section { title = "Customers", icon = "🔔", knobs = new[]
            {
                ...
                new Knob { key = "entranceStyle", label = "Entrance", def = 0f, min = 0f, max = 3f, step = 1f,
                           kind = KnobKind.Enum, options = new[] { "Walk", "Hop", "Pop", "Slide" },
                           Get = c => (float)(int)c.entranceStyle,
                           Set = (c, v) => c.entranceStyle = (EntranceStyle)(int)v },
